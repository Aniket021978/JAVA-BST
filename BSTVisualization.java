...
public class BSTVisualization extends JFrame implements ActionListener, KeyListener {
	...
	private JLabel labelInorder, labelPreorder, labelPostorder, labelHeight, labelDiameter;
	private JLabel ansInorder, ansPreorder, ansPostorder, ansHeight, ansDiameter;
	...
	private void initialize() {
		...
		// Diameter of BST label
		labelDiameter = new JLabel("BST Diameter : ");
		labelDiameter.setFont(new Font("Calibri", Font.BOLD, 20));
		topLeftPanel.add(labelDiameter);

		// Diameter of BST answer
		ansDiameter = new JLabel("0");
		ansDiameter.setFont(new Font("Calibri", Font.BOLD, 20));
		ansDiameter.setPreferredSize(new Dimension(50, 30));
		topLeftPanel.add(ansDiameter);
		...
		btnDeleteAll.setFont(new Font("Arial", Font.BOLD, 20));
		// btnDelete.setBounds(size.width - 130, 60, 100, 30);
		btnDeleteAll.addActionListener(this);
		topRightPanel.add(btnDeleteAll);
	...
	public void actionPerformed(ActionEvent evt) {
		if (evt.getSource() == btnDeleteAll) {
			deleteAll(); // Call delete all method
			return; // Exit early to avoid parsing input
		}
		if (tf.isEnabled()) {
			try {
				if (evt.getSource() == btnDelete && root == null) {
					JOptionPane.showMessageDialog(null, "The BST Is Already Empty");
					return;
				}
				...
				JOptionPane.showMessageDialog(null, "Please Enter Integer To Add/Delete.");
			}
		}
	}
	...
	public void deleteAll() {
		...
		root = null; // Clear the root to delete all nodes
		treePanel.removeAll(); // Remove all components from the tree panel
		treePanel.repaint(); // Refresh the tree panel
	...
	private void setInfo() {
		...
		if (height.root == 0) {
			...
			ansDiameter.setText(String.valueOf(diameter));
		}
	...
	// Calculate Diameter of BST using recursive method.
	private int calculateDiameter(Node root) {
		...
		int leftHeight = calculateHeight(root.left).root;
		int rightHeight = calculateHeight(root.right).root;

		// Diameter is max of:
		// 1. Diameter of left subtree
		// 2. Diameter of right subtree
		// 3. Height of left subtree + height of right subtree + 1 (for the root)
		return Math.max(Math.max(calculateDiameter(root.left), calculateDiameter(root.right)),
